...
Future<ExtensionProject?> loadExtensionProject(
  ...
  logger
      .info(() => '========= Loading jb extension project: $rootDir =========');
  ...
  final configContainer =
      await withCurrentDirectory(rootDir, () => JbConfigContainer(config));
  ...
  final extensionModel = await createJbExtensionModel(
      configContainer, rootDir, classpath, cache, jvmExecutor);

  // convert the [ExtensionTask]s and constructor data into Dartle [Task]s
  final tasks = extensionModel.extensionTasks.map((extensionTask) {
    return _createTask(
        jvmExecutor, classpath, extensionTask, absRootDir, cache);
...
Task _createTask(Sendable<JavaCommand, Object?> jvmExecutor, String classpath,
    ExtensionTask extensionTask, String absRootDir, DartleCache cache) {
  final runCondition = _runCondition(extensionTask, cache);
  return Task(_taskAction(jvmExecutor, classpath, absRootDir, extensionTask),
