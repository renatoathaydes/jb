  ...
  group('Incremental Compilation', () {
    test('can recompile only changed sources', () async {
      ...
      await assertDirectoryContents(
        ...
        ['Main.class', 'util', p.join('util', 'Util.class')],
      );

      final mainTimestamp = await File(
        p.join(rootDir.path, 'build', 'classes', 'Main.class'),
      ).lastModified();
      final utilTimestamp = await File(
        p.join(rootDir.path, 'build', 'classes', 'util', 'Util.class'),
      ).lastModified();

      final javaResult = await runJava(rootDir, [
        '-cp',
        p.join('build', 'classes'),
        'Main',
      ]);
      ...
      final javaResult2 = await runJava(rootDir, [
        '-cp',
        p.join('build', 'classes'),
        'Main',
      ]);
      ...
      final mainTimestamp2 = await File(
        p.join(rootDir.path, 'build', 'classes', 'Main.class'),
      ).lastModified();
      ...
      expect(
        mainTimestamp2.millisecondsSinceEpoch,
        equals(mainTimestamp.millisecondsSinceEpoch),
      );
      expect(
        utilTimestamp2.millisecondsSinceEpoch,
        greaterThan(utilTimestamp.millisecondsSinceEpoch),
      );
    });

    test(
      'can delete class and resource from incremental compilation',
      () async {
        final rootDir = await createTempFiles({
          'jbuild.yaml': '''
          source-dirs: [source]
    ...
    test(
      'does not perform incremental compilation if output changes',
      () async {
        final rootDir = await createTempFiles({
          'jbuild.yaml': 'source-dirs: [source]\noutput-dir: build/classes',
          p.join('source', 'util', 'Util.java'): '''
