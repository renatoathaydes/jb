...
void main() {
  group('Raw values tests', () {
    test('does not resolve raw strings', () {
      ...
      expect(
        resolveProperties({'foo': 'bar'}, {'bar': 'zort'}),
        equals({'foo': 'bar'}),
      );
    });
    ...
    test('does not resolve missing keys', () {
      expect(
        resolveProperties({'foo': '{{bar}}'}, {}),
        equals({'foo': '{{bar}}'}),
      );
      expect(
        resolveProperties({'foo': 'a{{bar}}b'}, {'foo': 'bar'}),
        equals({'foo': 'a{{bar}}b'}),
      );
    });
    ...
    test('does not bomb on nulls', () {
      ...
      expect(
        resolveProperties({'foo': '{{bar}}'}, {'bar': null}),
        equals({'foo': '{{bar}}'}),
      );
    });
    ...
    test('can resolve string properties', () {
      ...
      expect(
        resolveProperties({'foo': 'XY{{bar}}Z'}, {'bar': 'zort'}),
        equals({'foo': 'XYzortZ'}),
      );
      expect(
        resolveProperties({'foo': 'XY{{bar}}{{bar}}'}, {'bar': 'zort'}),
        equals({'foo': 'XYzortzort'}),
      );
      expect(
        resolveProperties({'foo': 'XY{{bar}}Z{{bar}}W'}, {'bar': '?'}),
        equals({'foo': 'XY?Z?W'}),
      );
      expect(
        resolveProperties(
